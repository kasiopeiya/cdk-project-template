...
| フロントエンドテストユーティリティ | Testing Library（React） |

#### テスト対象とディレクトリ

| レイヤー | テスト対象の「単体」 | プロダクションコード | テストコード |
| --- | --- | --- | --- |
| バックエンド | Lambda関数のhandler | `backend/src/handlers/` | `backend/src/handlers/__tests__/` |
| フロントエンド | コンポーネント / カスタムフック | `frontend/src/` | 各コンポーネントと同階層の `__tests__/` |
...
**本質的に重要な原則：** 必要なテストだけをやる。ブラックボックスの思想は「実装の詳細を検証するな」という意味であり、「実装を見るな」「テストケースを追加するな」という意味ではない。カバレッジ分析で未検証の重要な経路が見つかった場合は、テストケースを追加すべきである。
...
本プロジェクトでは、テストダブルに関する用語を以下のとおり定義する。コードレビューやテスト実装において、これらの用語を正確に使い分けること。

| 用語 | 定義 | 役割 |
| --- | --- | --- |
| **テストダブル** | テスト対象の依存を置き換える代用品の総称 | モック・スタブ等すべてを包含する上位概念 |
| **スタブ（stub）** | 事前に定義した固定レスポンスを返すだけのテストダブル | テスト対象にデータを**提供する**（入力側）。呼び出しの検証は行わない |
| **モック（mock）** | 固定レスポンスを返す機能に加え、呼び出しの有無・引数・回数を**検証する**テストダブル | テスト対象からの呼び出しが正しいことを**検証する**（出力側） |

> **判断基準：** そのテストダブルに対して `toHaveBeenCalledWith` 等の**検証（assert）を行うかどうか**で区別する。検証しない → スタブ、検証する → モック。
...
- カバレッジ計測で未検証の経路が見つかった場合、その経路がテスト対象（2.8節の基準）に該当するなら**テストケースを追加する**
